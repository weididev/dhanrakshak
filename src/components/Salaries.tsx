...
import { Plus, Trash2, Calendar, IndianRupee, CheckCircle2, Clock, Landmark, ArrowUpDown, Search, FileText } from 'lucide-react';
import { motion, AnimatePresence } from 'motion/react';
import { SalaryOffers } from './SalaryOffers';

export const Salaries = () => {
  const { salaries = [], assets = [], addSalary, deleteSalary } = useFinance();
  const [activeTab, setActiveTab] = useState<'income' | 'offers'>('income');
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-4">
        ...
        <div className="flex p-1 bg-[#141414] rounded-lg border border-[#1f1f1f] w-full md:w-auto self-start">
          <button
            onClick={() => setActiveTab('income')}
            className={`flex-1 md:flex-none px-6 py-2 rounded-md text-sm font-bold uppercase tracking-widest transition-all ${
              activeTab === 'income' 
          ...
          <button
            ...
            className={`flex-1 md:flex-none px-6 py-2 rounded-md text-sm font-bold uppercase tracking-widest flex items-center justify-center gap-2 transition-all ${
              activeTab === 'offers' 
                ? 'bg-[#2a2a2a] text-[#a855f7] shadow-sm' 
                : 'text-[#808080] hover:text-[#a855f7]'
            }`}
          >
