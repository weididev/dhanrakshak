...
import { Plus, Trash2, ShieldAlert, Landmark, TrendingUp } from 'lucide-react';
...
export function Assets() {
  ...
  const investmentTotal = assets.filter(a => !['emergency_fund', 'cash'].includes(a.type)).reduce((acc, a) => acc + a.amount, 0);
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center mb-8">
        ...
        >
          ...
          <span className="font-mono text-sm uppercase tracking-wider">{isAdding ? 'CANCEL' : 'ADD ASSET'}</span>
        </button>
      </div>
      ...
      {isAdding && (
        <div className="glass-panel p-6 rounded-xl neon-border mb-8 border-[#00ff9d]/30 animate-in fade-in slide-in-from-top-4">
          <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="md:col-span-3 flex justify-end mt-2">
              <button 
                type="submit"
                className="bg-[#00ff9d] text-[#050505] font-bold px-6 py-2 rounded-lg hover:bg-[#00ff9d]/80 transition-colors shadow-[0_0_15px_rgba(0,255,157,0.3)]"
              >
                SAVE ASSET
              </button>
            </div>
